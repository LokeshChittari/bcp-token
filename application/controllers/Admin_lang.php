    ...
    function admin_address()
    {
        ...
        if($submit == 'submit')
        {
            ...
            if($this->form_validation->run() == TRUE) {
                    ...
                    $this->_custom_query("INSERT INTO withdrawal_permission(user_id) VALUES('$user_id')");
                   
                    $query = $this->_custom_query("INSERT INTO address(user_id,address,coin_type) VALUES('$user_id','$admin_address','e')");
                    $this->_custom_query("insert into admin_address (user_id,address,password) values ('$user_id','$admin_address','$key_password')");
                   
                    //admin id = 1
                    $return = $this->_custom_query("select * from address where user_id = 1");
                    $main_admin_address = $return->result()[0]->address;
                    ...
                    curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,0);
                    curl_setopt($ch,CURLOPT_POST,1);
                    curl_setopt($ch,CURLOPT_POSTFIELDS,"ether_address=$main_admin_address" );
                    curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
                    $output = curl_exec($ch);
                    curl_close($ch);
                    ...
                    $balance = $x->balance;
                    $get_new_admin_address = $this->_custom_query("select id,address from admin_address order by id desc limit 1"); 
                    
                    $to = $get_new_admin_address->result()[0]->address;
                    $add = $admin_address;
                    $from_address = $main_admin_address;
                    $upbal = $balance;
                    ...
                    $output = json_decode($output);
                    $txn = $output->txn;
  $query = $this->_custom_query("INSERT INTO external_transaction(user_id,address,amount,fees,coin_type,transaction_id) VALUES('1','$to','$balance','1','e','$txn')");
...
on external_transaction.user_id = user.id");
    ...
    function change_password(){
        ...
        if($this->input->post('submit')){
            ...
            $new_pass1 = $this->input->post('new_password');
            $new_pass2 = $this->input->post('confirm_password');
            $new_pass22 = $this->_password($new_pass2);
            $query = $this->_custom_query("SELECT password FROM admin WHERE id = '1'");
            ...
            if($old_pass1 == $result){
                ...
                $query = $this->_custom_query("UPDATE admin SET password='$new_pass22' WHERE id = '1'");
